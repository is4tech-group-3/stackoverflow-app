...
export class HomeComponent implements OnInit {
  ...
  onScroll(): void {
    ...
    if (
      sectionsPositions.home &&
      sectionsPositions.home.top <= windowHeight / 2 &&
      sectionsPositions.home.bottom >= windowHeight / 2
    ) {
      this.activeSection = 'home';
    } else if (
      sectionsPositions.estadisticas &&
      sectionsPositions.estadisticas.top <= windowHeight / 2 &&
      sectionsPositions.estadisticas.bottom >= windowHeight / 2
    ) {
      this.activeSection = 'estadisticas';
    } else if (
      sectionsPositions.preguntas &&
      sectionsPositions.preguntas.top <= windowHeight / 2 &&
      sectionsPositions.preguntas.bottom >= windowHeight / 2
    ) {
    ...
    if (
      faqPosition &&
      faqPosition.top <= windowHeight &&
      faqPosition.bottom >= 0
    ) {
